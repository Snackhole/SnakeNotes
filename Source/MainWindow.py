...
from PyQt5.QtWidgets import QMainWindow, QInputDialog, QMessageBox, QFileDialog, QAction, QShortcut, QSplitter, QApplication
...
class MainWindow(QMainWindow):
    ...
    def CreateInterface(self):
        ...
        self.UpdateNotebookFormatAction = QAction("Update Notebook Format")
        self.UpdateNotebookFormatAction.triggered.connect(self.UpdateNotebookFormat)
        ...
        self.NotebookMenu.addSeparator()
        self.NotebookMenu.addAction(self.UpdateNotebookFormatAction)
    ...
    def UpdateNotebookFormat(self):
        OpenFileName = QFileDialog.getOpenFileName(caption="Update Notebook File Format", filter="Notebook files (*" + self.FileExtension + ")")[0]
        if OpenFileName != "":
            # Default Data for Root Pages
            DefaultRootData = {}
            DefaultRootData["Images"] = {}
            DefaultRootData["PageTemplates"] = {}
            DefaultRootData["Header"] = "# {PAGETITLE}"
            DefaultRootData["Footer"] = ""

            # Default Data for Sub Pages
            DefaultSubPageData = {}
            DefaultSubPageData["Images"] = None
            DefaultSubPageData["PageTemplates"] = None
            DefaultSubPageData["Header"] = None
            DefaultSubPageData["Footer"] = None

            # Load and Adjust JSON
            with open(OpenFileName, "r") as LoadFile:
                JSONString = LoadFile.read()
            Data = json.loads(JSONString)
            for RootKey, RootDefault in DefaultRootData.items():
                if RootKey not in Data["ObjectData"]:
                    Data["ObjectData"][RootKey] = RootDefault
            for SubPageData in Data["ObjectData"]["SubPages"]:
                self.UpdateNotebookSubPagesFormat(SubPageData, DefaultSubPageData)

            # Write Adjusted Data
            AdjustedJSONString = json.dumps(Data)
            with open(OpenFileName, "w") as WriteFile:
                WriteFile.write(AdjustedJSONString)

            # Display Message
            OpenFileNameShort = os.path.basename(OpenFileName)
            self.DisplayMessageBox("Finished updating the format of " + OpenFileNameShort + ".")

    def UpdateNotebookSubPagesFormat(self, CurrentSubPageData, DefaultSubPageData):
        for SubPageKey, SubPageDefault in DefaultSubPageData.items():
            if SubPageKey not in CurrentSubPageData["ObjectData"]:
                CurrentSubPageData["ObjectData"][SubPageKey] = SubPageDefault
        for SubPageData in CurrentSubPageData["ObjectData"]["SubPages"]:
            self.UpdateNotebookSubPagesFormat(SubPageData, DefaultSubPageData)
